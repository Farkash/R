...
burritos <- burritos[1:50,]
```

## About the data

There are `r nrow(burritos)` rows in this data set, with data taken from `r burritos %>% count(Neighborhood) %>% nrow()` neighborhoods and there are `r burritos %>% count(Burrito) %>% nrow()` burrito types.
...
```{r dist prep and plot of reviewers in one step, echo=FALSE, results=FALSE, fig.cap="Frequency Distribution of Reviewers"}
...
```{r Rating vs Cost plot, echo=FALSE, warning=FALSE}
